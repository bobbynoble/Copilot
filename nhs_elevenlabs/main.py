"""
NHS Translation Bot — ElevenLabs + Claude variant.
...
GET  /api/translate/audio/{task_id}/file — Download dubbed audio
...
from nhs_elevenlabs.agent import (
    LANG_CONFIG,
    detect_language,
    fetch_dubbed_audio,
    generate_tts,
...
from nhs_elevenlabs.models import (
    AudioDubbingRequest,
...
from nhs_elevenlabs.session_manager import session_manager
...
app = FastAPI(
    title="NHS Translation Bot (ElevenLabs)",
    description=(
        "AI-powered translation and audio dubbing for NHS patient communication. "
        "Powered by ElevenLabs (TTS + dubbing) and Claude (translation, language detection, "
        "signage reading, content moderation)."
    ),
    version="1.0.0",
...
async def create_session(body: dict) -> dict:
    ...
    lang_name = LANG_CONFIG.get(patient_lang, patient_lang)
    return {
...
async def get_session(session_id: str) -> dict:
    ...
    lang_name = LANG_CONFIG.get(session.patient_lang, session.patient_lang)
    return {
...
async def list_languages() -> list[LanguageOption]:
    return [LanguageOption(code=code, name=name) for code, name in LANG_CONFIG.items()]

@app.get("/api/voices", tags=["Translation"])
async def list_voices() -> list[dict]:
    voices = await get_voices()
    return [{"id": v["voice_id"], "name": v.get("name", v["voice_id"])} for v in voices]
...
async def translate_text(body: TextTranslationRequest) -> TextTranslationResponse:
    try:
        ...
        detected_name = LANG_CONFIG.get(source, source)
...
async def dubbing_status(task_id: str, target_lang: str = "EN_GB") -> DubbingStatusResponse:
    try:
        result = await get_dubbing_status(task_id, target_lang)
...
@app.get("/api/translate/audio/{task_id}/file", tags=["Dubbing"])
async def dubbing_file(task_id: str, target_lang: str = "EN_GB") -> Response:
    try:
        audio_bytes = await fetch_dubbed_audio(task_id, target_lang)
        return Response(content=audio_bytes, media_type="audio/mpeg")
    except Exception as exc:
        logger.exception("Failed to fetch dubbed audio for %s", task_id)
